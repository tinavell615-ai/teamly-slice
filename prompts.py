...
Позывной / операционный псевдоним, если им называют действующее лицо, = карточка персонажа.
...
**Антидубли и тождество (обязательно):**
- Перед create сверься со списком известных.
- Короткое имя при единственном полном («Том» при «Том Реддл») → **update** полной карточки, не create.
- Уточнение в скобках («Том (бармен)») — только если из текста ясно, что это **другой** человек; иначе вопрос.
- **Тождество позывной ↔ имя:** если в куске прямо сказано, что позывной и гражданское имя — один человек («X, он же Y», «настоящее имя — Y») → одна карточка (основное имя — как устойчивее назван в сюжете), второе — в properties.aliases / поле «Также известен», не вторая карточка.
- Если тождество **только подозревается**, а в тексте не сказано — **вопрос** с вариантами «один человек» / «разные», default — «разные» (безопаснее). Не сливать наугад.
