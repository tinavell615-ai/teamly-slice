...
**Антидубли имён (обязательно):**
- Перед create сверься со списком известных. Если короткое имя («Том») уже покрыто единственным полным («Том Реддл») — это **update** полной карточки, не create «Том».
- Если короткое имя может быть **другим** человеком (в тексте другая роль: бармен Том, солдат Том) — либо create с уточнением в названии: «Том (бармен)», либо (при сомнении) **вопрос** с вариантом по умолчанию «тот же Том Реддл» / «новый персонаж».
- Две карточки с пересекающимися именами без различия роли/контекста — запрещены.
