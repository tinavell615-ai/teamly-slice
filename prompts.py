...
PHASES: dict[int, dict[str, Any]] = {
    ...
    1: {
        ...
        "goal": "Извлечь персонажей и явные референсы по тестам вычленения.",
    },
    2: {
        "name": "Пространство",
        "tables": ["locations"],
...
### 👤 Персонаж
Кандидат, если: назван И (совершает действие ИЛИ является объектом действия, влияющего на сюжет).
Не кандидат: безымянные функции («стражники», «толпа»), категории без влияния.
Смена формы (живой→тень/призрак) — та же карточка, смена статуса. Новая карточка запрещена.
...
def _tables_hint(phase: int) -> str:
    ...
    return "Таблицы фазы: " + ", ".join(tables)
